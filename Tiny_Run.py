...
ROJO_NORMAL = (255, 0, 0)
NEGRO = (0, 0, 0)
NEGRO = (0, 0, 0) # color para el record
...
def obtener_record():
    try:
        conexion = sqlite3.connect("tiny_run.db")
        cursor = conexion.cursor()
        # Selecciona el puntaje mas alto
        cursor.execute("SELECT MAX(puntos) FROM puntuaciones")
        record = cursor.fetchone()[0]
        conexion.close()
        # Si no hay record  devuelve 0
...
try:
    ...
    fondo_juego_img = pygame.image.load("DNA.jpg").convert()
...
def juego_principal():
    ...
    while True:
        ...
        texto_record = fuente.render(f"Récord: {display_record}", True, NEGRO)
